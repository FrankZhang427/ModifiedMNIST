...
def main(version):
    ...
    p.status()
    print(x_train.shape)
    g = p.keras_generator_from_array(x_train, y_train, batch_size=x_train.shape[0])
    skew_1, skew_y_1 = next(g)
    print(skew_1.shape)
    skew_2, skew_y_2 = next(g)
    skew_1 = skew_1.reshape(x_train.shape)
    skew_2 = skew_2.reshape(x_train.shape)
    ###############################################################################
    # Elastic distortion
    ...
    x_train = np.concatenate((x_train, distort_1, distort_2, distort_3, skew_1, skew_2), axis=0)
    y_train = np.concatenate((y_train, y_train, y_train, y_train, skew_y_1, skew_y_2), axis=0)
    ###############################################################################
